...
app.post("/verify-password", authenticate, async (req, res) => {
  ...
  try {
    ...
    // Assuming empmodel stores plain text passwords (not recommended in production)
...
/* ---------------------- Update User Details ---------------------- */
app.patch("/update-user", authenticate, async (req, res) => {
  ...
  try {
    ...
    await user.save();
    res.status(200).json({ message: "User details updated", user: { username: user.username, email: user.email } });
  } catch (err) {
